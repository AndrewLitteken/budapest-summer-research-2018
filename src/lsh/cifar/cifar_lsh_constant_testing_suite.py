  ...
  for file_name in categories:
    ...
    for nClasses in nClasses_list:
      for unseen in unseen_list:
        ...
        test_images_raw = np.reshape(test_images_raw, [len(test_images_raw), 3, 32, 32])
        test_images_raw = np.transpose(test_images_raw, [0, 2, 3, 1])
        test_labels_raw = test[b"labels"]
